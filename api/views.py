...
from drf_yasg.utils import swagger_auto_schema # pyright: ignore[reportMissingImports]
...
# This code defines a DRF View class
#  called MyTokenObtainPairView, which inherits from TokenObtainPairView.
...
class ProfileView(generics.RetrieveUpdateAPIView):
    ...
    def get_serializer_context(self):
        return {'request': self.request}  # <--- Make sure to pass this context!
...
class LikePostAPIView(APIView):
    def post(self, request):
        ...
        if not user_id or not post_id:
            return Response({"message": "user_id and post_id are required."}, status=status.HTTP_400_BAD_REQUEST)
